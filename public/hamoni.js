...
class Value {
  constructor(appId, io, name, state) {
    ...
    this._io.on(`value-updated-${appId}-${name}`, state => {
      this._state = state;
  ...
  update(newState) {
    ...
    else if (typeof newState === "function" || typeof newState === "object")
      throw Error("State has an invalid type");
    else {
...
class HamoniSync {
  ...
  connect() {
    return new Promise((resolve, reject) => {
      this._io = lib$1(config.serverUrl, {
        query: {
          appId: this._appId,
      ...
      this._io.once(
        "connect_error",
        error => console.log("error connecting") || reject(error)
      );
    });
  }
  ...
  createList(name, state) {
    return new Promise((resolve, reject) => {
      if (
        typeof state.constructor === Array ||
        state === null ||
        state === undefined
      ) {
        reject("State is invalid");
